...
export default function AddNewFdModel({isUpdate,setIsUpdate}) {
  ...
  return (
    <> 
        ...
        <div       onClick={openModal} className="w-full  border border-dashed border-blue-500 rounded shadow px-4 py-3 flex items-center justify-center flex-col hover:bg-blue-500 hover:text-white transition-all duration-300 cursor-pointer text-sky-700  ">
          ...
          <div className="fixed inset-0 overflow-y-auto">
              ...
              >
                <Dialog.Panel className="w-full max-w-lg transform overflow-hidden rounded-2xl bg-white p-6 text-left align-middle shadow-xl transition-all">
                  ...
                  >
                    ...
                    <button  onClick={closeModal} type='button' className='text-xl p-2 bg-sky-100 rounded-full text-sky-700 cursor-pointer'>
                      <RiCloseLargeLine/>
                    </button>
                  </Dialog.Title>
                  <div className="mt-2">
                    <p className="text-sm text-gray-500">
                    Get Per day <b>0.1%</b> interest by Credixa
                    </p>
                  </div>
            ...
            <Form className='py-5'>
              <div className="mb-3">
                ...
                <Field type="text" name='apply_for' id='apply_for' className='w-full bg-transparent border border-sky-100 rounded-md py-3 px-4 outline-none' placeholder='Enter FD Purpose...' />
                <ErrorMessage className='text-red-500' component={'p'} name='apply_for' />
              </div>
              <div className="mb-3">
                <label htmlFor="amount">Amount</label>
                <Field type="number" name='amount' id='amount' className='w-full bg-transparent border border-sky-100 rounded-md py-3 px-4 outline-none' placeholder='Enter FD Amount' />
              ...
              <div className="mb-3">
                ...
                <Field  as="select" name='account' id='account' className='w-full bg-transparent border border-sky-500 rounded-md py-3 px-4 outline-none'   >
                 
                  {(() => {
  if (!user || !user.account_no) {
    return <option value="">No Accounts to select from</option>;
  }

  // if single account → convert to array
